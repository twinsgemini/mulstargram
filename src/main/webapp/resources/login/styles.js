...
$(document).ready(function () {
    ...
    $("#id").blur(function () {
        if ($('#id').val() == '') {
            ...
            $('#idMsg').text('4~12자의 영문, 숫자만 사용 가능합니다.');
            $('#idMsg').css('color', 'red');
        } else if ($('#id').val() != '') {
            var user_id = $('#id').val();
            $.ajax({
    ...
    $('form').on('submit', function () {
        ...
        //성별 확인
        // //라디오 버튼
        // if (member.user_gender[0].checked == false && member.user_gender[1].checked == false) {
        //     inval_Arr[6] = false;
        //     alert('성별을 확인하세요.');
        //     return false;
        // } else {
    ...
    $('#user_birth').blur(function () {
        ...
        if (dateStr.length <= 8) {
            ...
            if (year > yearNow || year < 1900) {
                $('#birthMsg').text('출생년도 확인해주세요');
                $('#birthMsg').css('color', 'red');
            }
            else if (month < 1 || month > 12) {
                $('#birthMsg').text('생일을 확인해주세요 ');
                $('#birthMsg').css('color', 'red');
            } else if (day < 1 || day > 31) {
                $('#birthMsg').text('날짜를 확인해주세요 ');
                $('#birthMsg').css('color', 'red');
