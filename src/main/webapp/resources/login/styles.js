				...
				function() {
					...
					$('form').on(
							...
							function() {
								...
								// 성별 확인
								// //라디오 버튼
								// if (member.user_gender[0].checked == false &&
								// member.user_gender[1].checked == false) {
								// inval_Arr[6] = false;
								// alert('성별을 확인하세요.');
								// return false;
								// } else {
					...
					$('#user_birth')
							.blur(
									function() {
										...
										var year = Number(dateStr.substr(0, 4)); // 입력한
										// 값의
										// 0~4자리까지
										// (연)
										var month = Number(dateStr.substr(4, 2)); // 입력한
										// 값의
										// 4번째
										// 자리부터
										// 2자리
										// 숫자
										// (월)
										var day = Number(dateStr.substr(6, 2)); // 입력한
										// 값
										// 6번째
										// 자리부터
										// 2자리
...
$("#id")
		.blur(
				function() {
					...
					$
							.ajax({
								url : '${pageContext.request.contextPath}/컨트롤러에서 지정/idCheck?userId='
										+ user_id,
								type : 'get',
								success : function(data) {
									if (data == 1) {
